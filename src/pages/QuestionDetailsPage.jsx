...
const QuestionDetailsPage = () => {
  ...
  const handleClick = () => {
    if (!posting) {
      ...
      requestBody.description &&
        axios
          .post(`${API_URL}/addAnswer`, requestBody)
  ...
  const checkIfCanEdit = () => {
    if (question.userId === user._id) {
      ...
      console.log(
        user.name,
        " Not the original poster, original is",
        question.postedBy
      );
    }
  };
  ...
  const handleDelete = () => {
    axios.delete(API_URL).then(navigate("/"));
  };
        ...
        <div className="margin-div card-list">
          ...
          <div className="skill-list">
            ...
            {question.skills.length === 0 ? (
              <p>No specific context</p>
            ) : (
              question.skills.map((skills) => {
                return <p className="skill-label">{skills}</p>;
              })
            )}
          </div>
          <hr className="hr"></hr>
          ...
          <div className="answers-div">
            ...
            {isLoggedIn && (
              ...
              >
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <button id="post-answer-button" onClick={handleClick}>
                    Post
                  </button>
                </div>
              </form>
            ...
            {isLoggedIn && (
              <button
                style={{ display: posting ? "none" : "block" }}
                id="new-answer-button"
                onClick={handleClick}
              >
                New Answer
              </button>
